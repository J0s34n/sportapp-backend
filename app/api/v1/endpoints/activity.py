...
def start_session(
    ...
    session = ActivitySession(
        ...
        started_at=_make_aware(body.started_at).replace(tzinfo=None),
    )
    db.add(session)
    db.commit()
    db.refresh(session)
...
def end_session(
    ...
    # Normalizar ambos datetimes a UTC naive para poder restar
    ended_at_naive   = _make_aware(body.ended_at).replace(tzinfo=None)
    started_at_naive = session.started_at.replace(tzinfo=None) \
        if session.started_at.tzinfo else session.started_at

    session.ended_at         = ended_at_naive
    ...
    session.duration_seconds = (ended_at_naive - started_at_naive).total_seconds()
...
def get_today_summary(
    ...
    return _build_summary(db, current_user, date.today())
